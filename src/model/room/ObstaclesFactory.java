...
public class ObstaclesFactory {
    ...
    private List<SimpleObject> getWalls(final Room room) {
        ...
        SimpleObject tmp;
        //TOP
        tmp = new Wall(UL_CORNER, GameObjectType.INVISIBLE_OBJECT, room);
        tmp.setBoundingBox(new BoundingBox(UL_CORNER, BR_CORNER.getX() - UL_CORNER.getX(), 1));
        walls.add(tmp);
        //RIGHT
        tmp = new Wall(new Point2D(BR_CORNER.getX(), UL_CORNER.getY()), GameObjectType.INVISIBLE_OBJECT, room);
        tmp.setBoundingBox(new BoundingBox(new Point2D(BR_CORNER.getX(), UL_CORNER.getY()), 1, BR_CORNER.getY() - UL_CORNER.getY()));
        walls.add(tmp);
        //BOTTOM
        tmp = new Wall(new Point2D(UL_CORNER.getX(), BR_CORNER.getY()), GameObjectType.INVISIBLE_OBJECT, room);
